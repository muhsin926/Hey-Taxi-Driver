...
const Chat = () => {
  ...
  const submitHandler = async (e) => {
    ...
    const { data } = await axios.post(
      `${import.meta.env.VITE_SERVER_DOMAIN}/api/driver/chat`,
      { chatInput, toUserId },
      {
  ...
  const getChater = async () => {
    ...
    const { data } = await axios.get(
      `${import.meta.env.VITE_SERVER_DOMAIN}/api/driver/getChater`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
    dispatch(setChaters(data));
  };

  const getMsgs = async (id) => {
    const token = localStorage.getItem("token");
    const { data } = await axios.get(
      `${import.meta.env.VITE_SERVER_DOMAIN}/api/driver/chat?passengerId=${id}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
  ...
  const pickReceiver = (passenger) => {
    dispatch(setToUserId(passenger._id));
    setChater(passenger);
    getMsgs(passenger._id);
    setInbox(true);
  };
  ...
  return (
    <div className="grid grid-cols-12 gap-2 ">
      <div className="sm:block hidden col-span-8 h-[30rem] border border-zinc-400 rounded-lg ">
